...
本平台是一个完整的 Web 数据驾驶舱系统，需要**后端服务器 + PostgreSQL 数据库**配合运行。根据你的需求，可以选择以下两种方式：

### 方式一：直接使用（推荐，无需部署）

> 如果你想直接查看和使用平台的全部功能（数据驾驶舱、智能问答等），**无需自己部署**。
>
> 📧 **联系作者，作者将为你启动服务器后端**，你只需在浏览器中打开网页即可使用。
>
> 适用于：快速体验系统功能、教学演示、临时查询数据分析结果。

### 方式二：自行部署

> 如果你想在自己的服务器上独立部署运行本系统，需要以下敏感配置信息：
>
> | 凭据 | 说明 |
> |------|------|
> | **PostgreSQL 数据库密码** | 用于连接数据库，包含 42 张业务表的读写权限 |
> | **DeepSeek API Key** | 用于智能问答模块的大语言模型调用 |
>
> 📧 **请联系作者获取数据库密码及 API Key**，联系时请说明使用场景（学术研究 / 教学演示 / 商业应用）。
>
> 适用于：独立运行、二次开发、数据定制、长期使用。

> ⚡ **简单来说：想直接用 → 联系作者开服务器；想自己部署 → 联系作者拿密码和 Key。**
...
> 📧 **联系作者获取数据库密码与 API Key**（参见[开始使用前请阅读](#️-开始使用前请阅读)）。
